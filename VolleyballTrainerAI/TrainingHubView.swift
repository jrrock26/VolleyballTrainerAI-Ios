...
struct TrainingScheduleView: View {
    ...
    private func trainingBlockRow(_ block: TrainingBlock) -> some View {
        let isCompleted = completedBlocks.contains(block.id)
        return TrainingScheduleRow(block: block, seconds: timers[block.id] ?? block.durationMinutes * 60,
