...
struct TeamManagementHubView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            viewModel.fetchTeams(context: modelContext)
        }
        .sheet(isPresented: $viewModel.isCreatingTeam) {
    ...
    private var teamListView: some View {
        VStack(spacing: 16) {
            ...
            if !viewModel.teams.filter({ $0.isArchived }).isEmpty {
                DisclosureGroup {
                    ForEach(viewModel.teams.filter { $0.isArchived }) { team in
                        TeamCardView(team: team, isArchived: true) {
                            withAnimation {
                                viewModel.selectedTeam = team
                            }
                        }
    ...
    private func quickActionsRow(team: TeamModel) -> some View {
        ...
        .sheet(isPresented: $viewModel.showCreateMatch) {
            CreateMatchView(viewModel: viewModel, team: team)
        }
        .sheet(isPresented: $viewModel.showCreateEvent) {
            CreateEventView(viewModel: viewModel, team: team)
        }
        .sheet(isPresented: $viewModel.isCreatingMember) {
            CreateMemberView(viewModel: viewModel, team: team)
        }
    }
    ...
    private func tabContent(team: TeamModel) -> some View {
        ...
        case .team:
            TeamInfoDetailView(team: team, viewModel: viewModel)
        case .roster:
            RosterListView(team: team, viewModel: viewModel)
        case .matches:
            MatchListView(team: team, viewModel: viewModel)
        case .stats:
            TeamStatsView(team: team)
        case .schedule:
            ScheduleView(team: team, viewModel: viewModel)
        case .chat:
            ChatListView(team: team, viewModel: viewModel)
        case .paperwork:
            PaperworkListView(team: team, viewModel: viewModel)
        }
    }
}
            ...
            HStack(spacing: 14) {
                ...
                ZStack {
                    Circle()
                        ...
                        .overlay(
                            Circle()
                                .stroke(NeonGlassStyle.neonGradient(startColor: .pink, endColor: .blue), lineWidth: 2)
                        )
                    ...
                    if let logoData = team.logoData, let uiImage = UIImage(data: logoData) {
                        ...
                        Image(systemName: team.level.iconName)
                            .font(.title2)
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 8) {
                        Label(team.level.rawValue, systemImage: team.level.iconName)
                            .font(.caption)
                            .foregroundColor(.blue)
                        
                        Label(team.season, systemImage: "calendar")
                            .font(.caption)
                            .foregroundColor(.gray)
                    }
                    
                    Text("\(team.players.count) players • \(team.coaches.count) coaches")
                        .font(.caption2)
                        .foregroundColor(.gray)
                }
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 16)
                            .stroke(
                                NeonGlassStyle.neonGradient(startColor: .pink.opacity(0.6), endColor: .blue.opacity(0.5)),
                                lineWidth: 1.5
                            )
                    )
            )
...
struct TeamInfoCard: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(team.name)
                    ...
                    .foregroundColor(.white)
                Text("\(team.level.rawValue) • \(team.season)")
                    .font(.caption)
                    .foregroundColor(.gray)
        ...
        .sheet(isPresented: $viewModel.isEditingTeam) {
            EditTeamView(viewModel: viewModel, team: team)
        }
    }
}
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 20)
                    .fill(isSelected ? Color.pink.opacity(0.3) : Color.black.opacity(0.5))
                    .overlay(
                        RoundedRectangle(cornerRadius: 20)
...
#Preview {
    TeamManagementHubView()
        .modelContainer(for: [TeamModel.self, TeamMember.self, MatchModel.self])
}