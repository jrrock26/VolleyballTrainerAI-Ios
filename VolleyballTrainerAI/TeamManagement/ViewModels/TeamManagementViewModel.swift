    ...
    func saveMatchResults(from engine: LiveScoringEngine, match: MatchModel,
        ...
        for setResult in engine.setHistory {
            let set = SetModel(
                ...
                homeScore: setResult.homeScore,
                awayScore: setResult.awayScore
            )
        ...
        // Save play by play events
        for entry in engine.playByPlayLog {
            let event = PlayEvent(
                setNumber: entry.setNumber,
                eventType: entry.eventType,
                description: entry.description,
                scoringTeam: entry.scoringTeam,
        ...
        for (playerID, stats) in engine.homePlayerStats {
            ...
            playerStats.attackAttempts = stats.attackAttempts
            playerStats.attackErrors = stats.attackErrors
            playerStats.aces = stats.aces
            playerStats.serveAttempts = stats.serveAttempts
            playerStats.serveErrors = stats.serveErrors
            playerStats.soloBlocks = stats.soloBlocks
            playerStats.blockAssists = stats.blockAssists
            playerStats.digs = stats.digs
...
struct NeonGlassStyle {
    ...
    static func neonGradient(startColor: Color = .pink, endColor: Color = .blue) -> LinearGradient {
        LinearGradient(
            gradient: Gradient(colors: [startColor.opacity(0.9), endColor.opacity(0.7)]),
